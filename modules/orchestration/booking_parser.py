# modules/orchestration/booking_parser.py
import re
...
class BookingDateTimeParser:
    ...
    def parse_date(self, user_input: str) -> Optional[datetime]:
        ...
        if user_input in ["tomorrow", "tmrw"]:
        ...
        for pattern, fmt in patterns:
            ...
            if match:
                try:
                    return datetime.strptime(fmt(match.groups()), "%Y-%m-%d")
                except:
                    continue
        return None
    ...
    def validate_booking(self, booking_datetime: datetime) -> Tuple[bool, Optional[str]]:
        """Check if booking date/time is valid (not past, within 90 days, within business hours)."""
        now = datetime.now(self.tz)
        if booking_datetime < now:
            return False, "Booking date is in the past"
