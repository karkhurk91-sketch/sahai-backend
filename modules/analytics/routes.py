...
async def get_activity(
    ...
    # Correct query: join with conversations to filter by organization
    msg_query = select(
        group_by.label("date"),
    ...
    # Similar fix for leads (leads has organization_id directly, so no join needed)
    lead_query = select(
        group_by.label("date"),
    ...
    # Top customers (join with conversations)
    top_query = select(
